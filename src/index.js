...
const reWriteJson = require('./reWriteJson.js')
const {
  parseProps,
  parseNpmName,
  generateEntry,
...
function formatComponentSchema(schema) {
  ...
  // debug("chema.screenshot,",schema.screenshot)
...
function getUsedComponentMetas(rootDir, lowcodeDir = 'lowcode', metaFilename, components) {
  ...
  // debug('Command %s metaPaths', metaPaths);
  // debug('Command %s components', components);
  ...
  return components
    ? components.filter((component) => {
        // debug('Command %s camel2KebabComponentName', camel2KebabComponentName(component));
...
module.exports = async (options, pluginOptions = {}) => {
  debug("transferMeta2Json1")
  const { registerUserConfig, registerCliOption,onHook } = options;
  ...
  const { setterMap, engineScope, npmClient,splitMeta,library } = pluginOptions;
  if (setterMap) {
  ...
  debug("transferMeta2Json2")

  onHook('before.build.run',(stats)=>{
    debug("rootDir",rootDir)
    reWriteJson.reWtiteJson(rootDir,library)
  })

  onHook('after.build.compile', (stats) => {
    // debug("transferMeta2Json3")
    transferMeta2Json(rootDir,splitMeta)
...
async function start(options, pluginOptions) {
  ...
  debug("PARSED_NPM_NAME.uniqueName",PARSED_NPM_NAME.uniqueName)
  const {
...
async function initLowCodeSchema(
  ...
  // debug("result[0]",JSON.stringify(result[0]))
...
async function bundleMetaV2(options, pluginOptions, execCompile, metaType) {
  ...
  debug("bundleMetaV2-end",metaPath)
...
async function bundleRenderView(options, pluginOptions, platform, execCompile) {
  ...
  // debug('_componentViews ', _componentViews);
  componentViews = `{${_componentViews
    .map((component) => {
      return `${component}: ${component}`;
    })
...
async function bundleComponentMeta(webPackConfig, options, pluginOptions, execCompile) {
  ...
  debug('Command %s usedDevComponents', usedDevComponents);
  ...
  onHook('after.build.compile', () => {
    ...
    usedDevComponents.forEach((comp) => {
      ...
      // debug("jsContentTarget",jsContentTarget)
      try {
...
function transferMeta2Json(rootDir,splitMeta) {
  debug("splitMeta",splitMeta)
    ...
    try {
      ...
      if(splitMeta){
        for (let i = 0; i < p.components.length; i++) {
          ...
          fse.outputFileSync(
            path.resolve(rootDir, `build/lowcode/${json.componentName}.json`),
            toJson(_json),
          );
        }
      }else{
        var _json = p.components
        if(_json.length===1){
          _json=_json[0]
        }
        fse.outputFileSync(
